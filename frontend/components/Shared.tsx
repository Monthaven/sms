...
export const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-medium border ${styles[status] || defaultStyle}`}>
      {status === 'Hot' && <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse"></span>}
      {status.toUpperCase()}
    </span>
  );
};
...
export const Avatar = ({ name, size = 'sm' }: { name: string, size?: 'sm' | 'md' | 'lg' }) => {
  ...
  return (
    <div className={`${sizeClasses[size]} rounded-lg flex items-center justify-center font-bold bg-gradient-to-br from-indigo-500 to-violet-600 text-white shadow-lg`}>
      {initials}
    </div>
  );
};
...
export const TypeBadge = ({
  ...
  <span
    className={`inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wider ${
      type === "Commercial" ? "text-indigo-300" : "text-emerald-300"
    }`}
  >
    {type === "Commercial" ? <Building2 size={10} /> : <MapPin size={10} />}
    {type}
...
export const Avatar = ({
  ...
  return (
    <div
      className={`${sizeClasses[size]} rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 text-white shadow-lg flex items-center justify-center font-bold`}
    >
      {initials}
    </div>
  );
