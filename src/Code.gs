...
function validateConfig_(context) {
  const missing = [];
  const required = [
    ['HOLDER_SPREADSHEET_ID', CONFIG.HOLDER_SPREADSHEET_ID],
    ['NOTION_TOKEN', CONFIG.NOTION.TOKEN],
    ['LEAD_STAGING_DB', CONFIG.NOTION.LEAD_STAGING_DB],
    ['CAMPAIGN_BATCHES_DB', CONFIG.NOTION.CAMPAIGN_BATCHES_DB],
    ['EZ_USER', CONFIG.EZ.USER],
    ['EZ_PASS', CONFIG.EZ.PASS]
  ];
  ...
  if (missing.length) {
    console.error(`[config] Missing Script Properties${context ? ' for ' + context : ''}: ${missing.join(', ')}`);
    return false;
  }

  console.log(`[config] Script Properties OK${context ? ' for ' + context : ''}.`);
  return true;
}

function validateConfig() {
  return validateConfig_('manual check');
}
...
function executeCampaignSend() {
  ...
  if (!validateConfig_('executeCampaignSend')) return;
...
function dryRunCampaignQueued() {
  if (!validateConfig_('dryRunCampaignQueued')) return;
